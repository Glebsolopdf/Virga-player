...
if [[ -f "$TARGET_DIR/build.sh" ]]; then
    read -p "Начать компилировать бинарник? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    if [[ -x "$TARGET_DIR/virga-player/bin/virga-player" ]]; then
        "$TARGET_DIR/virga-player/bin/virga-player"
    else
        error "Binary not found at $TARGET_DIR/virga-player/bin/virga-player. Build it first."
    fi
fi
