...
func (a *Artwork) drawImageInBox(screen tcell.Screen, x, y, w, h int, img image.Image, fade, pulse float64) {
	...
	for row := 0; row < h; row++ {
		...
		for col := 0; col < w; col++ {
			...
			topColor := rgbaToTcell(sampleCenterColor(img, srcX0, topSrcY0, srcX1, topSrcY1), bg, fade, pulse)
			botColor := rgbaToTcell(sampleCenterColor(img, srcX0, botSrcY0, srcX1, botSrcY1), bg, fade, pulse)
