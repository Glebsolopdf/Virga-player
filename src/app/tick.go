...
func (a *App) onTick(dt float64) {
	...
	if a.audioAnalyzer != nil {
		currentBands, ok := a.audioAnalyzer.Bands()
		if ok {
			bands = currentBands
	...
	if a.state.PlayerEnabled && a.state.Player != nil {
		a.state.Player.UpdatePosition(a.width, a.height, a.cfg.PlayerPosition)
	...
	frame.NewFrame(
		...
		a.cfg.PlayerPosition,
		a.cfg.LyricsPosition,
		a.cfg.LyricsDisplayMode,
		a.state.Lyrics,
		bands,
		footerPromptText,
	).Render(dt)
