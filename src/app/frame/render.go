...
func (f Frame) renderPlayerLyricsOverlay() {
	p := f.Player
	if p == nil || f.Lyrics == nil {
		return
	}
	f.Lyrics.SetRaw(p.SyncedLyrics)
	f.Lyrics.Render(f.Screen, f.Renderer, playerlyrics.RenderOptions{
		Mode:           f.LyricsDisplayMode,
		StaticPosition: f.LyricsPosition,
