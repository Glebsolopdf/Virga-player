...
type Player struct {
	...
	ArtworkURL   string
	SyncedLyrics string
	Position     settings.PositionPreset
}
...
func (p *Player) UpdatePosition(width, height int, preset settings.PositionPreset) {
	p.Position = preset
	bounds := playerlyrics.PlayerBounds(width, height, preset)
	...
	p.TextY = centerY - 3
	if p.Artwork != nil {
		p.Artwork.SetAnchor(centerX, centerY)
	}
}
