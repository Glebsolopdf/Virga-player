...
func (p *Page) Render(screen tcell.Screen, renderer *renderer.Renderer, width, height int) {
	...
	case sectionNotifications:
		subtitle = "Notification history. Open this page to mark unread items as read."
	case sectionNone:
	...
	switch p.Section {
	case sectionNotifications:
		p.renderNotifications(screen, renderer, width, height)
	default:
		items := p.menuItems()
		startY := 8
	...
	case sectionNotifications:
		helpText = "Esc: go back  |  Enter: back  |  s: top menu"
	}
	if p.Config.Debug {
