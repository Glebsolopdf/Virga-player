...
func Visual(cfg *settings.Config, selected, delta int) bool {
	...
	case 4:
		prev := cfg.PlayerPosition
		options := settings.PositionPresetOptions()
		index := PositionIndex(cfg.PlayerPosition)
		index = (index + delta + len(options)) % len(options)
		cfg.PlayerPosition = options[index]
		if !settings.IsLyricsPositionAllowed(cfg.PlayerPosition, cfg.LyricsPosition) {
			cfg.LyricsPosition = settings.PositionBelowPlayer
		}
		return cfg.PlayerPosition != prev
	case 5:
		prev := cfg.Player
