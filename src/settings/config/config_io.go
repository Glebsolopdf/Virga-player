...
func LoadOrCreateConfig() (*Config, bool, error) {
	...
	if err := json.Unmarshal(data, &raw); err == nil {
		...
		_, hasNotificationsEnabled := raw["notifications_enabled"]
		_, hasNotifyUnreadToast := raw["notify_unread_toast"]
		if !hasNotificationsEnabled || !hasNotifyUnreadToast {
			missingLyricsFields = true
		}
	}
...
func (c *Config) MarshalJSON() ([]byte, error) {
	return json.Marshal(savedConfigJSON{
		...
		NotifyUnreadToast:      c.NotifyUnreadToast,
	})
}
...
func (c *Config) UnmarshalJSON(data []byte) error {
	...
	c.Player = raw.Player
	if raw.NotificationsEnabled == nil {
		c.NotificationsEnabled = true
	} else {
		c.NotificationsEnabled = *raw.NotificationsEnabled
	}
	if raw.NotifyUnreadToast == nil {
		c.NotifyUnreadToast = false
	} else {
		c.NotifyUnreadToast = *raw.NotifyUnreadToast
	}
	return nil
}
