...
func (c *Config) normalize() {
	...
	c.normalizePositions()
	c.normalizeLyricsDisplayMode()
...
func (c *Config) normalizePositions() {
	c.PlayerPosition = normalizePosition(c.PlayerPosition, PositionCenter)
	c.LyricsPosition = normalizePosition(c.LyricsPosition, PositionBelowPlayer)
	c.LyricsPosition = ResolveLyricsPosition(c.PlayerPosition, c.LyricsPosition)
}

func normalizePosition(value, fallback PositionPreset) PositionPreset {
	switch value {
	case PositionTopRight, PositionBottomRight, PositionTopCenter, PositionCenter, PositionBelowPlayer:
		return value
	default:
		return fallback
	}
...
func (c *Config) normalizeLyricsDisplayMode() {
	switch c.LyricsDisplayMode {
	case LyricsDisplayStatic, LyricsDisplayDynamic:
	default:
		c.LyricsDisplayMode = LyricsDisplayStatic
	}
}
